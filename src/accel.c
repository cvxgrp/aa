...
void BLAS(gemm)(const char *transa, const char *transb, blasint *m, blasint *n,
                blasint *k, scs_float *alpha, scs_float *a, blasint *lda,
                scs_float *b, blasint *ldb, scs_float *beta, scs_float *c,
                blasint *ldc);
...
scs_float *solve_accel_linsys(Accel *a, scs_int len) {
  ...
  BLAS(gemv)
  ("Trans", &twol, &bk, &onef, a->dF, &twol, a->sol, &one, &zerof, a->scratch,
   &one);
  BLAS(trsv)("Upper", "Trans", "NotUnitDiag", &bk, a->R, &bk, a->scratch, &one);
  /* scratch = R^-1 * scratch */
  BLAS(trsv)
  ("Upper", "NoTrans", "NotUnitDiag", &bk, a->R, &bk, a->scratch, &one);
  /* sol = g */
  memcpy(a->sol, a->g, sizeof(scs_float) * 2 * a->l);
  /* sol = sol - dG * scratch */
  BLAS(gemv)
  ("NoTrans", &twol, &bk, &negOnef, a->dG, &twol, a->scratch, &one, &onef,
   a->sol, &one);
  RETURN a->sol;
}
...
scs_int solve_with_gels(Accel *a, scs_int len) {
  ...
  /* solve dF scratch = f */
  BLAS(gels)
  ("NoTrans", &twol, &blen, &one, X, &twol, a->scratch, &twol, wrk, &worksize,
   &info);
  /* sol = g */
  memcpy(a->sol, a->g, sizeof(scs_float) * 2 * l);
  /* sol = sol - dG * scratch */
  BLAS(gemv)
  ("NoTrans", &twol, &blen, &negOnef, a->dG, &twol, a->scratch, &one, &onef,
   a->sol, &one);
  scs_free(wrk);
  scs_free(X);
  RETURN info;
...
void update_factorization(Accel *a, scs_int idx) {
  ...
  BLAS(gemv)
  ("Trans", &twol, &bk, &onef, a->dFold, &twol, u, &one, &zerof, w, &one);
  BLAS(trsv)("Upper", "Trans", "NotUnitDiag", &bk, a->R, &bk, w, &one);
  // BLAS(gemv)("Trans", &twol, &bk, &onef, a->Q, &twol, u, &one, &zerof, w,
  // &one);
  ...
  BLAS(gemv)
  ("NoTrans", &twol, &bk, &negOnef, a->dFold, &twol, tmp, &one, &onef, u, &one);
  scs_free(tmp);
  /* nrm_u = ||u|| */
  ...
  for (i = k; i > idx + 1; --i) {
    ...
    // BLAS(rot)(&twol, &(Q[2 * l * (i-2)]), &one, &(Q[2 * l * (i-1)]), &one,
    // &c, &s);
  }
  ...
  for (i = idx + 1; i < k - 1; ++i) {
    ...
    // BLAS(rot)(&twol, &(Q[2 * l * i]), &one, &(Q[2 * l * (i+1)]), &one, &c,
    // &s);
  }
